...
def search_by_date():
    try:
        ...
        directory_export_search = os.path.join(
            os.path.dirname(my_path), 
            f'Export_Files/[InventoryHCM]_{today}_DetailSearchIn_{date_search}.csv')
...
def search_by_week():
    try:
        ...
        # Get script detail directory
        directory_query_detail = os.path.join(
            my_path, "Scripts/queryScript_DetailHCM_searchbyWeek.txt"
            )
        # Get export directory for total csv file
        directory_export_search = os.path.join(
            os.path.dirname(my_path), 
            f'Export_Files/[InventoryHCM]_{today}_DetailSearchIn_{year_search}-{week_search}.csv')
